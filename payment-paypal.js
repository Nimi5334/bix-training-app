...
export async function initPayPal(clientId) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Load SDK with Hosted Fields components
    ...
    if (submitBtn) {
      submitBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          // Capture order (backend handles card token)
          const captureResponse = await fetch('/.netlify/functions/capture-paypal-order', {
            ...
            body: JSON.stringify({
              ...
              amount,
              duration,
              cardToken: nonce, // Pass tokenized card data
            }),
          });
          if (!captureResponse.ok) throw new Error('Failed to process payment');
